"""
Rightmove Sales API Trigger Probe
=================================

Focused active recon probe for Rightmove sale search pages.

It tests likely triggers that may emit first-party listing-search API requests:
  - baseline idle after page load
  - next-page navigation
  - sort changes
  - optional manual user interactions

The goal is to identify the strongest sale-side API request shape so the sale
search scraper can prefer the best API payload instead of relying on looser DOM
merging when avoidable.

Examples:
  python3 Scraper/recon/rightmove_sales_api_trigger_probe.py \
  ...
  python3 Scraper/recon/rightmove_sales_api_trigger_probe.py \
    --search-url "https://www.rightmove.co.uk/property-for-sale/find.html?searchLocation=London&useLocationIdentifier=true&locationIdentifier=REGION%5E87490&radius=5.0&_includeSSTC=on" \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Actively probe hidden Rightmove sales API triggers.")
    parser.add_argument(
        "--search-url",
        help="Rightmove sale search URL. If omitted, the browser opens the sale homepage for manual navigation.",
    )
    parser.add_argument(
        "--wait-seconds",
        type=float,
...
def prompt_manual_ready():
    input(
        "\nOpen a Rightmove sales search, handle cookies / CAPTCHA if needed, "
        "then press Enter here to start the sales trigger probe..."
    )
...
def _listing_score(node):
    ...
    if any(key in node for key in ("propertyUrl", "url", "detailUrl")):
        score += 1
    if any(key in node for key in ("bedrooms", "bathrooms", "propertyType", "images")):
        score += 1
    return score
...
def _scan_sale_paths(node, prefix="", findings=None):
    if findings is None:
        findings = []
    if isinstance(node, dict):
        for key, value in node.items():
            path = f"{prefix}.{key}" if prefix else key
            if any(keyword in key.lower() for keyword in SALE_KEYWORDS):
                if isinstance(value, (str, int, float, bool)) and value not in ("", None):
                    findings.append({"path": path, "value": _normalise_space(str(value))})
            _scan_sale_paths(value, path, findings)
    elif isinstance(node, list):
        for index, item in enumerate(node[:5]):
            _scan_sale_paths(item, f"{prefix}[{index}]", findings)
    return findings


def _summarise_sale_paths(listing_nodes):
    counter = Counter()
    samples = defaultdict(list)
    for node in listing_nodes[:20]:
        for finding in _scan_sale_paths(node):
...
def _summarise_response_body(body_text):
    ...
    return {
        ...
        "sale_field_candidates": _summarise_sale_paths(listing_nodes)[:20],
    }
...
def _action_manual_window(driver, seconds):
    print(
        "\nManual trigger window started. Interact with the sales search page now "
...
def build_report(start_url, action_results):
    ...
    sale_field_candidates = defaultdict(list)
    for summary in response_body_summaries:
        ...
        for item in summary.get("sale_field_candidates", []):
            if len(sale_field_candidates[item["path"]]) < 4:
                sale_field_candidates[item["path"]].extend(item["samples"])
    ...
    report = {
        ...
        "analysis": {
            ...
            "response_sale_field_candidates": [
                {
                    "path": path,
                    "samples": list(dict.fromkeys(samples))[:4],
                }
                for path, samples in sale_field_candidates.items()
            ],
        },
    }
...
def build_recipe(report):
    lines = []
    lines.append("RIGHTMOVE SALES API TRIGGER PROBE")
    lines.append("=" * 50)
    ...
    sale_fields = analysis.get("response_sale_field_candidates", [])
    if sale_fields:
        lines.append("\nRESPONSE SALE FIELD CANDIDATES:")
        for item in sale_fields[:12]:
            lines.append(f"  • {item['path']}: {item['samples']}")

    if not analysis.get("successful_request_count", 0):
        lines.append("\nCONCLUSION:")
        lines.append("  No successful sale listing-search API request was observed during this probe.")
        lines.append("  If that happens, keep the search scraper conservative and DOM/source-backed.")
...
def save_report(report, output_dir):
    ...
    json_path = output_path / f"rightmove_sales_api_trigger_probe_{timestamp}.json"
    txt_path = output_path / f"rightmove_sales_api_trigger_probe_{timestamp}.txt"
...
def main():
    ...
    try:
        ...
        if not wait_for_cards(driver):
            raise RuntimeError(f"Sales cards did not appear: {driver.current_url}")
