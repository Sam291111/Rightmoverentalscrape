...
def _extract_price_qualifier(*values):
    candidates = (
        "Guide Price",
        "Offers in Excess of",
        "Offers Over",
...
def _extract_listing_from_api(node, page_index):
    ...
    listing_update = node.get("listingUpdate") if isinstance(node, dict) else None
    added_or_reduced = None
    if isinstance(listing_update, dict):
        added_or_reduced = listing_update.get("listingUpdateReason") or listing_update.get("addedOrReduced")
    if not added_or_reduced:
        added_or_reduced = _first_scalar_for_keys(node, ("addedOrReduced",))
    price_qualifier = _nested_value(node, "price", "displayPrices")
    if isinstance(price_qualifier, list) and price_qualifier:
        first_display = price_qualifier[0]
        if isinstance(first_display, dict):
            price_qualifier = first_display.get("displayPriceQualifier")
        else:
            price_qualifier = None
    lozenge_types = []
    lozenge_model = node.get("lozengeModel") if isinstance(node, dict) else None
    if isinstance(lozenge_model, dict):
        for item in lozenge_model.get("matchingLozenges", []) or []:
            if isinstance(item, dict) and item.get("type"):
                lozenge_types.append(_normalise_space(str(item["type"])))
    lozenge_types = list(dict.fromkeys([item for item in lozenge_types if item]))
    ...
    return {
        ...
        "price_qualifier": _extract_price_qualifier(price_qualifier, display_price),
        ...
        "tenure": _normalise_tenure(tenure_value),
        "added_or_reduced": _normalise_space(str(added_or_reduced)) if added_or_reduced else None,
        "auction": bool(node.get("auction")) if isinstance(node, dict) and node.get("auction") is not None else None,
        "lozenge_types": lozenge_types,
        "new_home": "NEW_HOME" in lozenge_types if lozenge_types else None,
...
def _choose_best_api_attempt(api_attempts, dom_cards, page_index):
    ...
    if best is None:
        ...
        raise RuntimeError(
            "Sales search API failed for all candidate URLs:\n"
...
def merge_listing_data(api_listing, dom_listing):
    ...
    if not merged.get("price_qualifier"):
        merged["price_qualifier"] = _extract_price_qualifier(merged.get("price_text"))
    ...
    if merged.get("tenure"):
        merged["tenure"] = _normalise_tenure(merged["tenure"])
    return merged
