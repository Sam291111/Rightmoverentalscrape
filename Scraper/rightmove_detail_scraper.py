...
Enriches the search-stage dataset by visiting each listing page and extracting:
  - full property image URLs
  - floorplan URLs
  - EPC URLs
  - description text
  - key features text
  - structured facts such as tenure / size / council tax / parking / garden
  - lat/lng from page source

Example:
  python3 Scraper/rightmove_detail_scraper.py --limit 10
  python3 Scraper/rightmove_detail_scraper.py --search-results-json Scraper/output/rightmove_search_results_20260324_155603.json
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Enrich Rightmove search results with detail-page data.")
    parser.add_argument(
        "--search-results-json",
...
def setup_browser(block_images=True):
...
def prompt_manual_ready():
    input(
        ...
        "then press Enter here to start the detail scraping pass..."
    )
...
def _recreate_browser(driver, landing_url, reprompt, block_images):
    _safe_quit(driver)
    driver = setup_browser(block_images=block_images)
...
def _navigate_with_recovery(driver, url, block_images, reprompt):
    try:
        if not _driver_window_available(driver):
            return _recreate_browser(driver, url, reprompt=reprompt, block_images=block_images)
        driver.get(url)
        return driver
...
def _latest_detail_run_dir(output_dir):
    candidates = sorted(
        Path(output_dir).glob("detail_run_*"),
...
def _resolve_run_dir(output_dir, run_dir_arg, resume):
    ...
    if resume:
        ...
        if latest:
            return latest
        raise FileNotFoundError("No previous detail run directory found to resume.")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return Path(output_dir) / f"detail_run_{timestamp}"
...
def _write_raw_page(run_dir, raw_page):
    ...
    file_path = raw_dir / f"rightmove_detail_{raw_page['listing_id'] or 'unknown'}.json"
...
def _latest_search_results_json():
    candidates = sorted(
        DEFAULT_OUTPUT_DIR.glob("rightmove_search_results_*.json"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    return candidates[0] if candidates else None
...
def load_search_results(path_arg, limit=None):
    ...
    if not search_json or not search_json.exists():
        raise FileNotFoundError("No search results JSON found. Pass --search-results-json or run the search scraper first.")
...
def _canonical_photo_url(url):
    ...
    if not url:
        return None
    if "property-photo" not in url.lower():
        return None
...
def _canonical_floorplan_url(url):
    ...
    if not url:
        return None
    if "property-floorplan" not in url.lower():
        return None
...
def _extract_clean_price_text(text):
    if not text:
        return None
...
def _normalise_tenure(value):
    ...
    if not text:
        return None
    lowered = text.lower()
    for candidate in TENURE_CANDIDATES:
        if candidate.lower() in lowered:
            return candidate
    return text


def _extract_tenure(*values):
    for value in values:
        tenure = _normalise_tenure(value)
        if tenure in TENURE_CANDIDATES:
            return tenure
    return None
...
"""
    ...
    snapshot["epc_urls"] = list(dict.fromkeys(_normalise_space(url) for url in snapshot.get("epc_urls", []) if _normalise_space(url)))
    snapshot["fact_pairs"] = [
        {
...
def _extract_structured_detail(snapshot, source_patterns):
    ...
    price_text = _extract_clean_price_text(snapshot["body_text"]) or _extract_clean_price_text(snapshot["title"])
    ...
    tenure = _normalise_tenure(facts.get("tenure")) or _extract_tenure(
        snapshot["body_text"],
        snapshot["h1"],
        snapshot["title"],
    )
    council_tax = facts.get("council tax")
    parking = facts.get("parking")
    garden = facts.get("garden")
    ...
    epc_rating = _extract_epc_rating(content_text, snapshot["body_text"], key_features_text, description, snapshot["title"], snapshot["h1"])
    ...
    return {
        ...
        "postcode": _first_valid_postcode(clean_location, clean_display_address, snapshot["body_text"]),
        ...
        "investment_text": _normalise_space(investment_phrase) if investment_phrase else None,
        "luxury": True if luxury_phrase else None,
        "luxury_text": _normalise_space(luxury_phrase) if luxury_phrase else None,
        "latitude": lat,
        "longitude": lng,
...
def _merge_search_and_detail(search_row, detail_row):
    ...
    if merged.get("detail_price_text"):
        merged["price_text"] = merged["detail_price_text"]
    if not merged.get("price_amount") and merged.get("price_text"):
        match = re.search(r"£\s*([\d,]+)", merged["price_text"])
        if match:
            merged["price_amount"] = int(match.group(1).replace(",", ""))
...
def save_outputs(output_dir, enriched_results, raw_path, metadata):
    ...
    json_path = output_path / f"rightmove_enriched_results_{timestamp}.json"
    with json_path.open("w", encoding="utf-8") as handle:
        json.dump(dataset, handle, indent=2, ensure_ascii=False)

    csv_path = output_path / f"rightmove_enriched_results_{timestamp}.csv"
...
def main():
    ...
    driver = setup_browser(block_images=args.block_images)

    try:
        if not search_results:
            raise RuntimeError("No search results to enrich.")
